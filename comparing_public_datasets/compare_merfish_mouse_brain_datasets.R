  ...
  for (metric in metrics) {
    ...
    for (reference_cell_type in cell_types_of_interest) {
      for (target_cell_type in cell_types_of_interest) {
        ...
        metric_data2D_subset$value <- (metric_data2D_subset$value) - (median(metric_data2D_subset[metric_data2D_subset[["dataset"]] == "cortex", "value"], na.rm = TRUE))
        metric_data3D_subset$value <- (metric_data3D_subset$value) - (metric_data3D_subset[metric_data3D_subset[["dataset"]] == "cortex", "value"])
  ...
  for (metric in metrics) {
    ...
    for (reference_cell_type in cell_types_of_interest) {
      for (target_cell_type in cell_types_of_interest) {
        ...
        # No data
        if (is.na(metric_data3D_subset[metric_data3D_subset[["dataset"]] == "hypothalamus", "value"]) ||
            all(is.na(metric_data2D_subset[metric_data2D_subset[["dataset"]] == "hypothalamus", "value"]))) {
          tally <- 0
        }
        # Tally = 1 if 3D value is in box plot
        else if (check_if_value_is_between_q1_and_q3(
          metric_data3D_subset[metric_data3D_subset[["dataset"]] == "hypothalamus", "value"],
          metric_data2D_subset[metric_data2D_subset[["dataset"]] == "hypothalamus", "value"])) {
          tally <- 1
        }
        # Tally = 2 if 3D value is above box plot and both 3D value and 2D median value are positive
        else if (
          metric_data3D_subset[metric_data3D_subset[["dataset"]] == "hypothalamus", "value"] >
          median(metric_data2D_subset[metric_data2D_subset[["dataset"]] == "hypothalamus", "value"], na.rm = TRUE) &&
          median(metric_data2D_subset[metric_data2D_subset[["dataset"]] == "hypothalamus", "value"], na.rm = TRUE) > 0
        ) {
          tally <- 2
        }
        # Tally = 3 if 3D value is below box plot but both 3D value and 2D median value are positive
          ...
          metric_data3D_subset[metric_data3D_subset[["dataset"]] == "hypothalamus", "value"] <
          median(metric_data2D_subset[metric_data2D_subset[["dataset"]] == "hypothalamus", "value"], na.rm = TRUE) &&
          metric_data3D_subset[metric_data3D_subset[["dataset"]] == "hypothalamus", "value"] > 0
        ) {
          tally <- 3
        }
...
pdf("merfish_3D_vs_2D_comparison.pdf", width = 20, height = 10)
...
setwd("~/R/plots/public_data/merfish_comparison")
pdf("merfish_3D_vs_2D_comparison_no_box_plot.pdf", width = 25, height = 10)
...
setwd("~/R/plots/public_data/merfish_comparison")
pdf("merfish_3D_vs_2D_comparison_heatmaps.pdf", width = 6, height = 5)
...
setwd("~/R/plots/public_data/merfish_comparison")
pdf("merfish_3D_vs_2D_comparison_bar.pdf", width = 8, height = 3.6)
...
# Consistent
setwd("~/R/plots/public_data/merfish_comparison")
pdf("merfish_3D_vs_2D_comparison_consistent_example_MS.pdf", width = 4.2, height = 2.8)
print(fig_metric_3D_vs_2D_comparison$FCO_AUC$`ASC/INC`) # Consistent
dev.off()

# Over
setwd("~/R/plots/public_data/merfish_comparison")
pdf("merfish_3D_vs_2D_comparison_over_example_ACIN.pdf", width = 4.2, height = 2.8)
print(fig_metric_3D_vs_2D_comparison$AMD$`ASC/EXC`) # Over
dev.off()

# Under
setwd("~/R/plots/public_data/merfish_comparison")
pdf("merfish_3D_vs_2D_comparison_under_example_AMD.pdf", width = 4.2, height = 2.8)
print(fig_metric_3D_vs_2D_comparison$CL_AUC$`INC/INC`) # Under
dev.off()

# Inconsistent
setwd("~/R/plots/public_data/merfish_comparison")
pdf("merfish_3D_vs_2D_comparison_inconsistent_example_CL.pdf", width = 4.2, height = 2.8)
print(fig_metric_3D_vs_2D_comparison$ANC_AUC$`OPC/OGC`) # inconsistent
dev.off()
